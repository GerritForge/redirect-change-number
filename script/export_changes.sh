...
function export_changes_number_from_eclipse_foundation {
    ...
    git ls-remote $SOURCE_PROJECT_GIT_URL | grep changes | awk  '{print $2}' | awk -F '/' '{print $4}' | uniq | sort -n | awk 'NF{print $0 ",'${DESTINATION_PROJECT}'"}'  > $DESTINATION_CSV_FILE.csv && \
    echo "Number of changes exported from jgit $(wc -l $DESTINATION_CSV_FILE)"
}
