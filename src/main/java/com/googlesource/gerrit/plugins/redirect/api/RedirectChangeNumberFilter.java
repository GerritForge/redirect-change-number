  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try {
      ...
      Optional<String> maybeRedirectURL =
          maybeXFHostHeader
              .filter(xfh -> xfh.equalsIgnoreCase(ECLIPSE_GERRITHUB_IO_HOST))
              .filter(xfh -> maybeXFPortHeader.isPresent() && maybeXFProtoHeader.isPresent())
              .flatMap(
                  xfh ->
      ...
      if (maybeRedirectURL.isPresent()) {
        ...
        if (!areAllXForwardedHeadersInRequest(
            maybeXFHostHeader, maybeXFPortHeader, maybeXFHostHeader)) {
          LOGGER.warn(
