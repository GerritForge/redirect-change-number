  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    try {
      ...
      Boolean areForwardedHeadersInRequest =
          areAllXForwardedHeadersInRequest(
              maybeForwardedHost, maybeForwardedPort, maybeForwardedProto);

      Optional<Integer> maybeChangeNumber = extractChangeNumberFromURI(httpRequest.getRequestURI());

      if (areForwardedHeadersInRequest) {
        if (maybeForwardedHost.get().equalsIgnoreCase(ECLIPSE_GERRITHUB_IO_HOST)) {
          Optional<String> maybeRedirectURL =
              maybeChangeNumber
                  .flatMap(changeNumber -> findProjectNameByChangeNumber(maybeChangeNumber.get()))
                  .map(
                      projectName ->
                          buildRedirectURL(
      ...
      if (!areForwardedHeadersInRequest && maybeChangeNumber.isPresent()) {
        LOGGER.warn(
